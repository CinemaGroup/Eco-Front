...
const StorageCreateFolder: FC<IStorageActionsItem> = ({ folderPath, setIsShow }) => {
	const {
	...
	return (
		<div className={styles.folder}>
			...
			<form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
				<Field
					{...registerInput('name', {
						required: 'Название папки обязательное поле',
					})}
					labelColor="black"
					variant="manage"
					...
					error={errors.name}
				/>
				<button className={styles.add} type='submit'>Создать</button>
			</form>
		</div>
	)
}
